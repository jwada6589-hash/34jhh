...
export default function AuthView({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    if (regData.fullName && regData.phone && regData.password && regData.password === regData.confirmPassword) {
      setAuthState('authenticated');
      onSuccess();
    }
  };
      ...
      <div className="p-6 flex-1">
        {mode === 'login' ? (
          <form onSubmit={handleLogin} className="space-y-4 max-w-sm mx-auto">
            ...
            <button type="submit" className="w-full bg-[#055C33] text-white font-bold py-4 rounded-xl shadow-md hover:bg-[#044727] transition mt-6">
              تسجيل الدخول
            </button>
          </form>
        ) : (
