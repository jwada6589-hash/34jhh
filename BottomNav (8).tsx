...
export default function BottomNav({ currentView, onViewChange, cartCount }: any) {
    ...
    return (
      ...
      >
        ...
        <div className={`relative p-2 rounded-full transition-all duration-300 ${isActive ? '-translate-y-1' : 'group-hover:-translate-y-0.5'}`}>
          ...
          <Icon 
            className="w-[24px] h-[24px] stroke-[2] relative z-10 transition-colors duration-300" 
            color={isActive ? activeColor : '#9ca3af'}
            fill={isActive ? activeFill : 'none'} 
            fillOpacity={isActive ? 0.2 : 0} 
          />
          {badge > 0 && (
            <span className="absolute -top-0.5 -right-0.5 bg-red-500 border-2 border-white text-white text-[8px] font-bold w-4 h-4 flex items-center justify-center rounded-full z-20 shadow-sm">
              {badge}
            </span>
          )}
        </div>
        ...
        {isActive ? (
          ...
          <span className="text-[10px] font-bold text-gray-400 transition-colors duration-300 group-hover:text-gray-500">
            {label}
          </span>
        )}
      </button>
  ...
  return (
    <>
      ...
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 w-max bg-white/95 backdrop-blur-md px-4 py-2 flex justify-center items-center gap-2 z-50 shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-gray-100 rounded-full">
