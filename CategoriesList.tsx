...
import { MOCK_CATEGORIES, MOCK_BRANCHES } from './mockData';
...
export default function CategoriesList() {
  ...
  const handleAddCategory = () => {
    ...
    const newCat: Category = {
      id: `cat-${Date.now()}`,
      name: newCatName,
      image: newCatImage || '📁', // Mock fallback
    };
  ...
  const handleDelete = (id: string) => {
    const branchesCount = MOCK_BRANCHES.filter(b => b.categoryId === id).length;
    if (branchesCount > 0) {
      alert('لا يمكن حذف هذا القسم لأنه يحتوي على فروع.');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredCategories.map(category => {
          const branchesCount = MOCK_BRANCHES.filter(b => b.categoryId === category.id).length;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-4 border-b border-gray-100">
              <h3 className="font-bold text-lg">إضافة قسم جديد</h3>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">صورة القسم (إيموجي حالياً)</label>
