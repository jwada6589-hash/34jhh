...
import { MOCK_BRANCHES, MOCK_CATEGORIES, MOCK_PRODUCTS } from './mockData';
...
export default function ProductsList() {
  ...
  const category = branch ? MOCK_CATEGORIES.find(c => c.id === branch.categoryId) : undefined;
  
  const [products, setProducts] = useState<Product[]>(MOCK_PRODUCTS.filter(p => p.branchId === branchId));
  ...
  const handleAddProduct = () => {
    ...
    const newProduct: Product = {
      ...
      image: newProdImage || '📦', // Mock fallback
      isAvailable,
      options: [...options],
    };
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-gray-900/50 backdrop-blur-sm sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col animate-in slide-in-from-bottom sm:zoom-in-95 duration-200">
            ...
            <div className="p-4 overflow-y-auto space-y-5">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">صورة المنتج (إيموجي حالياً)</label>
