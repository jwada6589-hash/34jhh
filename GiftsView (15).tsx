...
import { MOCK_GIFTS, MOCK_GIFT_REQUESTS } from './mockData';
...
export default function GiftsView() {
  ...
  const [requests, setRequests] = useState<GiftRequest[]>(MOCK_GIFT_REQUESTS);
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6 animate-in fade-in duration-300">
      ...
      <div className="mt-6">
        <Routes>
          <Route path="/" element={<GiftsList gifts={gifts} setGifts={setGifts} />} />
          <Route path="/requests" element={<GiftRequestsList requests={requests} setRequests={setRequests} gifts={gifts} setGifts={setGifts} />} />
        </Routes>
      </div>
    </div>
  );
