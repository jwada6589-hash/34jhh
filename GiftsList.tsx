...
export default function GiftsList({ gifts, setGifts }: GiftsListProps) {
  ...
  const handleSave = () => {
    ...
    const giftData: Gift = {
      ...
      quantity: qty,
      isDisabled,
      redemptionCount: editingGiftId ? gifts.find(g => g.id === editingGiftId)?.redemptionCount || 0 : 0,
    };

    if (editingGiftId) {
      setGifts(prev => prev.map(g => g.id === editingGiftId ? giftData : g));
    } else {
      setGifts([giftData, ...prev]);
    }
    
    setIsFormOpen(false);
  };

  const toggleStatus = (id: string, currentIsDisabled: boolean) => {
    setGifts(prev => prev.map(g => g.id === id ? { ...g, isDisabled: !currentIsDisabled } : g));
  };

  const handleDelete = () => {
    if (isDeleteOpen) {
      setGifts(prev => prev.filter(g => g.id !== isDeleteOpen));
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-gray-900/50 backdrop-blur-sm sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-lg shadow-xl overflow-hidden max-h-[90vh] flex flex-col animate-in slide-in-from-bottom sm:zoom-in-95 duration-200">
            ...
            <div className="p-4 overflow-y-auto space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">صورة الهدية (إيموجي حالياً)</label>
