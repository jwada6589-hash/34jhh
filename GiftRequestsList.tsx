...
import { Gift, GiftRequest, GiftRequestStatus, REQUEST_STATUS_DEF } from './types';

interface GiftRequestsListProps {
...
export default function GiftRequestsList({ requests, setRequests, gifts, setGifts }: GiftRequestsListProps) {
  ...
  const handleNextStep = (reqId: string, currentStatus: GiftRequestStatus) => {
    const statusDef = REQUEST_STATUS_DEF[currentStatus];
    if (!statusDef.next) return;

    setRequests(prev => prev.map(r => r.id === reqId ? { ...r, status: statusDef.next! } : r));
  };

  const handleCancel = () => {
    if (isCancelModalOpen) {
      setRequests(prev => prev.map(r => r.id === isCancelModalOpen ? { ...r, status: 'CANCELLED' } : r));
