...
import { MOCK_CATEGORIES, MOCK_BRANCHES, MOCK_PRODUCTS } from './mockData';
...
export default function BranchesList() {
  ...
  const [branches, setBranches] = useState<Branch[]>(MOCK_BRANCHES.filter(b => b.categoryId === categoryId));
  ...
  const handleAddBranch = () => {
    ...
    const newBranch: Branch = {
      ...
      name: newBranchName,
      image: newBranchImage || '📁', // Mock fallback
    };
  ...
  const handleDelete = (id: string) => {
    const productsCount = MOCK_PRODUCTS.filter(p => p.branchId === id).length;
    if (productsCount > 0) {
      alert('لا يمكن حذف هذا الفرع لأنه يحتوي على منتجات.');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredBranches.map(branch => {
          const productsCount = MOCK_PRODUCTS.filter(p => p.branchId === branch.id).length;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">صورة الفرع (إيموجي حالياً)</label>
