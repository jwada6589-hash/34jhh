...
export default function CartView({ items, updateQuantity, onViewChange, clearCart }: any) {
  ...
  const handleConfirmOrder = () => {
    ...
    // Save back to profile to remember their latest checkout details
    updateUserProfile(formData);
    
    // Create new order
    const now = new Date();
    const formattedDate = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')} ${now.getFullYear()}-${(now.getMonth()+1).toString().padStart(2, '0')}-${now.getDate().toString().padStart(2, '0')}`;
