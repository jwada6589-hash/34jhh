...
import { MOCK_PRODUCTS, MOCK_CATEGORIES, MOCK_BRANCHES } from '../products/mockData';
...
export default function OffersView() {
  ...
  const branchesForCat = MOCK_BRANCHES.filter(b => b.categoryId === selCategoryId);
  const productsForBranch = MOCK_PRODUCTS.filter(p => p.branchId === selBranchId);
  const selectedProduct = MOCK_PRODUCTS.find(p => p.id === selProductId);
  ...
  const saveOffer = () => {
    ...
    if (editingOfferId) {
      setOffers(prev => prev.map(o => o.id === editingOfferId ? offerData : o));
    } else {
  ...
  const toggleStatus = (id: string, currentIsDisabled: boolean) => {
    setOffers(prev => prev.map(o => o.id === id ? { ...o, isDisabled: !currentIsDisabled } : o));
  };

  const handleDelete = () => {
    if (isDeleteOpen) {
      setOffers(prev => prev.filter(o => o.id !== isDeleteOpen));
